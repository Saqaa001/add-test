...
with st.form(key="question_form"):
    ...
    # Apply Changes Button in the first column
    with col1:
        submit_button = st.form_submit_button(label="✅ Apply Changes")

    # Reset Form Button in the second column
    with col2:
...
# Reset Form behavior
if reset_button:
    # Reset all inputs and session state
    st.session_state["Question"] = ""  # Clear the question input
    st.session_state["LatexMap"] = {}  # Clear Latex map
    st.session_state["ModifiedText"] = ""  # Clear the modified text
    st.session_state["FinalQuestion"] = ""  # Clear the final question
    # Clear answers
    for key in ["A", "B", "C", "D"]:
        st.session_state[key] = ""
